...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.dataset_s = args.dataset_s
        self.dataset_t = args.dataset
        self.beta = args.beta
        ...
        *********************************************************
        ...
        if self.alg_type in ['lightgcn']:
            ...
            dataset_s = str(args.dataset_s)
            dataset_t = str(args.dataset)
            read_file = './embedding/' + dataset_s
            np_u_g_embeddings = np.load(read_file + '_user_embedding.npz.npy')
            np_T = np.load('./Tr_' + dataset_s + '_2_' + dataset_t + '.npy')
            print('############################### np: Load source domain user embedding.')

            tensor_u_g_embeddings = tf.convert_to_tensor(np_u_g_embeddings)
            print('############################### tensor: Load source domain user embedding.')
            # # 直接转换tensorflow
            # tensor_T = tf.convert_to_tensor(np_T)

            tf_T = tf.placeholder(tf.float32, shape=np_T.shape)
            # output_T = tf_T
            with tf.Session() as sess:
                print('############################### tensor: session')
                # 使用feed_dict将数据喂给占位符
                tensor_T = sess.run(tf_T, feed_dict={tf_T: np_T})

            u_g_embeddings_s = tf.Variable(tensor_u_g_embeddings, name='u_g_embeddings_s', trainable=False)
            T = tf.Variable(tensor_T, name='T', trainable=False)
            U_s = u_g_embeddings_s
            U_ts = tf.transpose(T) @ U_s
            print('$$$$$$$$$$$$$$$$$$$$$$$$$ cross user embedding: ', U_ts)

            # self-attention
            # 获取batch大小
            # b_size = self.user_batch_size
            # # print('########################batch size: ', b_size)
            # # print('#########################user_embedding:',user_embedding.shape)
            att_emb = tf.concat([tf.expand_dims(self.ua_embeddings, 1),
                                 tf.expand_dims(U_ts, 1)], 1)  # [,4]
            hidden_factor = 64
            selfAttention = SelfAttention(hidden_factor)
            att_emb = att_emb
            # print('####################att_emb: ', att_emb)
            for i in range(5):
                att_emb = selfAttention(att_emb)
            att_user_emb = att_emb[:, 1]

            # self.ua_embeddings = self.ua_embeddings + self.beta * U_ts
            self.ua_embeddings = self.ua_embeddings + self.beta * att_user_emb
        ...
        *********************************************************
        ...
        print('$$$$$$$$$$$$$$$$$$$$$$$$$ user embedding: ', self.u_g_embeddings)
    ...
    def _create_variables(self):
        ...
        weight_saver = tf.train.import_meta_graph(pretrain_path + 'model.meta')
        ...
        with tf.Session() as sess:
            weight_saver.restore(sess, self.read_file_t + 'model')
            ue, ie = sess.run([user_embeddings, item_embeddings])
        ...
        print("restore LightGCN network done")
        print("===========================")
    ...
    def _create_lightgcn_embed(self):
        ...
        print('$$$$$$$$$$$$$$$$$$$$$$$$$ gcn user embedding: ', u_g_embeddings)
...
def load_pretrained_data():
    u_pretrain_path = f'./embedding/{args.dataset}_user_embedding.npz.npy'
    i_pretrain_path = f'./embedding/{args.dataset}_item_embedding.npz.npy'
    try:
        pretrain_data = dict()
        user_emb = np.load(u_pretrain_path)
        item_emb = np.load(i_pretrain_path)
        pretrain_data = {'user_embed': user_emb, 'item_embed': item_emb}
